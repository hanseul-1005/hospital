...
function popOpen(num, classPrefix, no, name, date, as, note) {
	var modalBg = $(num);
...
function goAdd() {
	location.href="equipment.windy?menu=add";
}

function goSearch() {
	var name = document.getElementById('search_name').value;
	
	location.href="equipment.windy?menu=list&name="+name;
}
...
function goDelete(no) {
	...
	$.ajax({
		type: "POST",
		url: "equipment.windy?mode=delete", 
		data: param,
		error: ajaxFailed,
...
function goModify() {
	...
	var param = "&no="+no+"&name="+name+"&date="+date+"&as="+as+"&note="+note;
		
	$.ajax({
		type: "post", 
		url: "equipment.windy?mode=update", 
		data: param,
		async: false, 
		dataType: 'text', 
		error: ajaxFailed,
        ...
        <div class="wrapper_main_contents" >
			...
			<div class="wrapper_board_contents">
				<div class="wrapper_board">
					<span class="span_board_title">장비 관리</span>
						...
						<table class="tb_basic tb_management">
							...
							<tbody>
								<%for(int i=0; i<listEquipment.size(); i++) {
									EquipmentModel model = listEquipment.get(i);
								%>
								<tr>
									<td onClick="javascript: popOpen('.modal-bg1', '.modal_700_420-wrap1', <%=model.getNo() %>, '<%=model.getName() %>', '<%=model.getDate() %>', '<%=model.getAs() %>', '<%=model.getNote() %>');"><%=i+1 %></td>
									<td onClick="javascript: popOpen('.modal-bg1', '.modal_700_420-wrap1', <%=model.getNo() %>, '<%=model.getName() %>', '<%=model.getDate() %>', '<%=model.getAs() %>', '<%=model.getNote() %>');");"><%=model.getName() %></td>
									<td onClick="javascript: popOpen('.modal-bg1', '.modal_700_420-wrap1', <%=model.getNo() %>, '<%=model.getName() %>', '<%=model.getDate() %>', '<%=model.getAs() %>', '<%=model.getNote() %>');");"><%=model.getDate() %></td>
									<td onClick="javascript: popOpen('.modal-bg1', '.modal_700_420-wrap1', <%=model.getNo() %>, '<%=model.getName() %>', '<%=model.getDate() %>', '<%=model.getAs() %>', '<%=model.getNote() %>');");"><%=model.getAs() %></td>
									<td>
        ...
        <div class="wrapper_popup_contents">
			<div class="wrapper_popup_top">
				<div class="wrapper_popup_top_center">
					<span class="span_popup_title">장비 수정</span>
				</div>
			</div>
			...
			<div class="wrapper_popup" style="margin-top: 10px;">
				...
				<table class="popup_tb_border">
					...
					<tr>
						<th>A/S 연락처</th>
						<td><input type="text" id="as" class="input_text"></td>
					</tr>
					<tr>
						<th>비고</th>
