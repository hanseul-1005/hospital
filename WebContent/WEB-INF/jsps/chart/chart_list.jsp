...
function goAdd() {
	location.href="room.windy?menu=add";
}

function goSearch() {
	var name = document.getElementById('search_name').value;
	
	location.href="room.windy?menu=list&name="+name;
}
        ...
        <div class="wrapper_main_contents" >
			...
			<div class="wrapper_board_contents">
				<div class="wrapper_board">
					<span class="span_board_title">병실 관리</span>
						...
						<table class="tb_basic tb_management">
							...
							<tbody>
								<%for(int i=0; i<listRoom.size(); i++) {
									RoomModel model = listRoom.get(i);
								%>
								<tr>
									<td onClick="javascript: popOpen('.modal-bg1', '.modal_700_420-wrap1', 
										<%=model.getNo() %>, '<%=model.getCode() %>', '<%=model.getCodeNo() %>', '<%=model.getName() %>', '<%=model.getCnt() %>');"><%=i+1 %></td>
									<td onClick="javascript: popOpen('.modal-bg1', '.modal_700_420-wrap1', 
										<%=model.getNo() %>, '<%=model.getName() %>', '<%=model.getCodeNo() %>', '<%=model.getName() %>', '<%=model.getCnt() %>');"><%=model.getName() %></td>
									<td onClick="javascript: popOpen('.modal-bg1', '.modal_700_420-wrap1', 
										<%=model.getNo() %>, '<%=model.getName() %>', '<%=model.getCodeNo() %>', '<%=model.getName() %>', '<%=model.getCnt() %>');"><%=model.getCode() %></td>
									<td onClick="javascript: popOpen('.modal-bg1', '.modal_700_420-wrap1', 
										<%=model.getNo() %>, '<%=model.getName() %>', '<%=model.getCodeNo() %>', '<%=model.getName() %>', '<%=model.getCnt() %>');"><%=model.getCodeNo() %></td>
									<td onClick="javascript: popOpen('.modal-bg1', '.modal_700_420-wrap1', 
										<%=model.getNo() %>, '<%=model.getName() %>', '<%=model.getCodeNo() %>', '<%=model.getName() %>', '<%=model.getCnt() %>');"><%=model.getCnt() %></td>
									<td>
        ...
        <div class="wrapper_popup_contents">
			<div class="wrapper_popup_top">
				<div class="wrapper_popup_top_center">
					<span class="span_popup_title">병실 수정</span>
				</div>
			</div>
